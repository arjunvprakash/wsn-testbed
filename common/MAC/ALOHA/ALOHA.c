...
{
	return "Backffs,TotalFrames,TotalRetries,TotalFailures,PDR,TotalDrops";
}

int MAC_getMetricsData(uint8_t *buffer, uint8_t addr)
{
	const MAC_Data data = metrics.data[addr];
	int rowlen = sprintf(buffer, "%d,%d,%d,%d,%d", data.backoffs, data.frames, data.retries, data.failures, data.frames > 0 ? (((data.frames - data.failures - data.drops) * 100) / data.frames)  : 0, data.drops);
	sem_wait(&metrics.mutex);
