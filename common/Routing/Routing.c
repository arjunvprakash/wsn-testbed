...
void Routing_init(Routing* r, unsigned char addr) {
...
int Routing_recv(Routing* r, unsigned char* msg_buffer) {
...
int Routing_tryrecv(Routing* r, unsigned char* msg_buffer) {
...
int Routing_timedrecv(Routing* r, unsigned char* msg_buffer, unsigned int timeout) {
	// Timeout festlegen
	struct timespec ts;
...
int Routing_send(Routing* r, unsigned char addr, unsigned char* data, unsigned int len) {
	...
	if (msg.data == NULL) {
		fprintf(stderr, "Routing_send: malloc error!\n");
		exit(EXIT_FAILURE);
	}
...
int Routing_Isend(Routing* r, unsigned char addr, unsigned char* data, unsigned int len) {
	// Nachricht setzen
	sendMessage msg;
	msg.addr = addr;
	msg.len = len;
