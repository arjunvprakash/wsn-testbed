    ...
    if (ctrl == CTRL_MAC)
    {
        ...
        for (uint8_t i = metrics.minAddr; i <= metrics.maxAddr; i++)
        {
            ...
            if (data.sent > 0 || data.recv > 0)
            {
                ...
                int rowLen = sprintf(row, "%ld,%d,%d,%d,%d,%d\n", (long)timestamp, config.self, i, (data.sent + metrics.data[0].broadcast), data.recv, data.recv > 0 ? (data.latency / data.recv) : 0);
                if (extraLen > 0)
                {
    ...
    else if (ctrl == CTRL_ROU)
    {
        ...
        for (uint8_t i = metrics.minAddr; i <= metrics.maxAddr; i++)
        {
            ...
            if (data.sent > 0 || data.recv > 0)
            {
                ...
                int rowLen = sprintf(row, "%ld,%0d,%d,%d,%d,%d,%d\n", (long)timestamp, config.self, i, data.sent, data.recv, data.numHops, data.recv > 0 ? (data.totalLatency / data.recv) : 0);
                if (extraLen > 0)
                {
                    rowLen += snprintf(row + strlen(row), sizeof(row) - strlen(row), ",%s", extra);
                }

                rowLen += snprintf(row + strlen(row), sizeof(row) - strlen(row), ",%s", data.path == NULL ? (uint8_t *)"" : data.path);
...
int ProtoMon_Routing_sendMsg(uint8_t dest, uint8_t *data, unsigned int len)
{
    ...
    uint8_t footer[5];
    sprintf(footer, "%02d", config.self);
    uint8_t footerSize = strlen(footer);

    uint16_t extDataLen = overhead + len + footerSize;
...
int ProtoMon_Routing_recvMsg(Routing_Header *header, uint8_t *data)
{
    ...
    if (ctrl == CTRL_MSG)
    {
        ...
        uint8_t footerSize = (numHops * 3) - 1;
        uint8_t datalen = len - overhead - footerSize;
        ...
        // Extract path
        uint8_t *path = (uint8_t *)malloc(footerSize * sizeof(uint8_t));
        if (!path)
        {
            logMessage(ERROR, "malloc failed for path.\n");
            exit(EXIT_FAILURE);
        }
        ...
        routingMetrics.data[src].path = path;
        sem_post(&routingMetrics.mutex);
...
int ProtoMon_MAC_recv(MAC *h, unsigned char *data)
{
    ...
    // if (dest != ADDR_BROADCAST) // exclude broadcasts - beacons
    {
        ...
        if (routingOverhead && isMsg) // Monitor only msg packets
        {
            ...
            if (len < (MAX_PAYLOAD_SIZE - 3))
            {
                ...
                sprintf(pathAppend, "%s%02d", (numHops == 1 ? "" : pathSeparator), config.self);
                footerSize = strlen(pathAppend);
    ...
    memcpy(data, temp, len - overhead + footerSize);
