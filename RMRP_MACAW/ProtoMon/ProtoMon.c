    ...
    if (ctrl == CTRL_MAC)
    {
        ...
        for (uint8_t i = metrics.minAddr; i <= metrics.maxAddr; i++)
        {
            ...
            if (data.sent > 0 || data.recv > 0)
            {
                ...
                int rowLen = sprintf(row, "%ld,%d,%d,%d,%d,%d", (long)timestamp, config.self, i, (data.sent + metrics.data[0].broadcast), data.recv, data.recv > 0 ? (data.latency / data.recv) : 0);
                if (extraLen > 0)
                {
    ...
    else if (ctrl == CTRL_ROU)
    {
        ...
        for (uint8_t i = metrics.minAddr; i <= metrics.maxAddr; i++)
        {
            ...
            if (data.sent > 0 || data.recv > 0)
            {
                ...
                int rowLen = sprintf(row, "%ld,%0d,%d,%d,%d,%d,%d", (long)timestamp, config.self, i, data.sent, data.recv, data.numHops, data.recv > 0 ? (data.totalLatency / data.recv) : 0);
                if (extraLen > 0)
                {
                    rowLen += snprintf(row + strlen(row), sizeof(row) - strlen(row), ",%s", extra);
                }
                ...
                {
                    printf("### Path[%02d]: %s\n", i, data.path);
                    // path = (uint8_t *)malloc(strlen(data.path));
                ...
                rowLen += snprintf(row + strlen(row), sizeof(row) - strlen(row), ",%s", path);
...
int ProtoMon_Routing_sendMsg(uint8_t dest, uint8_t *data, unsigned int len)
{
    ...
    // Footer to track the path. path[0] = src
    uint8_t footer[5];
    uint8_t footerSize = snprintf(footer, sizeof(footer), "%02d", config.self);

    uint16_t extDataLen = overhead + len + 1 + footerSize;

    uint8_t extData[extDataLen];
    ...
    *temp = '\0';
    ...
    // Set path: self
    memcpy(temp, footer, footerSize);
    temp += footerSize;

    int ret = Original_Routing_sendMsg(dest, extData, extDataLen);
...
int ProtoMon_Routing_recvMsg(Routing_Header *header, uint8_t *data)
{
    ...
    if (ctrl == CTRL_MSG)
    {
        ...
        temp += datalen + 1; // Skip '\0'
        printf("### Data: %s (%d)\n", data, datalen);

        // Calculate footer length : len(pathString)
        uint8_t footerSize = getPathLen(numHops);
        *(temp + footerSize + 1) = '\0';
        // Extract path
        uint8_t path[footerSize];
        strcpy(path, temp);
        // memcpy(path, temp, footerSize);
        temp += footerSize;
        ...
        memcpy(routingMetrics.data[src].path, path, footerSize);
        routingMetrics.data[src].path[footerSize + 1] = '\0';
        sem_post(&routingMetrics.mutex);
...
int ProtoMon_MAC_send(MAC *h, unsigned char dest, unsigned char *data, unsigned int len)
{
    ...
    uint16_t extDataLen = overhead + len;
    uint8_t extData[extDataLen];
    uint8_t *temp = extData;
    ...
    time_t start = time(NULL);
    int ret = Original_MAC_sendMsg(h, dest, extData, extDataLen);
    ...
    if (dest != ADDR_BROADCAST) // exclude broadcasts - beacons
    {
        ...
        if (routingOverhead && isMsg) // Monitor only msg packets
        {
            ...
            if (len < (MAX_PAYLOAD_SIZE - 10))
            {
                ...
                // printf("### numHops:%d pathLen:%d cur path: %s\n", numHops, pathLen, curPath);
            }
            else
            {
                logMessage(ERROR, "Unable to append path. Packet full!\n");
            }
        }
    }
    else
