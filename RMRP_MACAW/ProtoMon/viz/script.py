...
def draw_edges_from_path(df_path, ax, df_rssi, pos=None, directed=True, config=None):
    ...
    for _, row in df_path.iterrows():
        ...
        for j in range(len(nodes) - 1):
            ...
            rssi_value = df_rssi.loc[df_rssi['key'] == key, 'RSSI']
            edgeLabel = rssi_value.iloc[0] if not rssi_value.empty else ''
            if config is not None:
                edgeLabel = ''
            edgeAlpha = 1.0  # Set edge transparency (can be modified based on conditions)
            if not G1.has_edge(int(source), int(address)):
                G1.add_edge(source, address, label=edgeLabel)
    pos1 = nx.circular_layout(G1) if pos is None else pos
    nx.draw(G1, pos1, with_labels=True, node_size=nodeSize, ax=ax, node_color=nodeColor)
...
if os.path.exists(network_csv):    
    ...
    if not df_path.empty:
        print(df_path)
        G1 = draw_edges_from_path(df_path, ax[i], df_neighbour)
        
        # Show the plot
        ax[i].set_title("Data Path")
        i += 1
    ...
    if not df_path.empty:
        ...
        if len(missing_nodes) == 0 and img is not None:   
            ...
            draw_edges_from_path(df_path, ax[i], df_neighbour, pos=pos3,config=map_config)
